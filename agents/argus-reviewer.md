...
- Security surface: injection sinks (shell, SQL, path, format) fed by external input, missing authorization or validation on new paths, secrets exposed or logged, unsafe deserialization, time-of-check/time-of-use races on new file operations, weakened crypto or randomness. External input includes any value the change does not itself constrain — CLI arguments, environment variables, config values, file names and contents, network data — and an unconstrained value interpolated into a shell or remote-shell command line is a sink even when today's callers look benign: report it at the severity the exposure warrants instead of suppressing it as unlikely. A pre-existing weakness qualifies only if the change widens it.
