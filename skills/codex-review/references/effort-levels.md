...
A finder or verifier dispatch that dies or returns nothing parseable is re-dispatched once. A verifier is ALSO a failed dispatch when its response is parseable but incomplete: it must return exactly one verdict per candidate in its group, so if any input candidate has no matching verdict block, the omitted candidate would silently vanish — re-dispatch the verifier once for the whole group. If it fails again (dead, unparseable, or still missing a candidate's verdict): for a finder, add to the Verdict's Explanation "lens L<n> did not complete — coverage is partial"; for a first-pass verifier, treat its unjudged candidates as unverified, noting "N candidate(s) at <location> could not be verified" in the Explanation. Never silently absorb a coverage loss.
...
A coverage loss must NEVER be laundered into a confident clean pass. If any finder lens or first-pass verifier failed twice AND synthesis would otherwise emit `patch is correct` with no confirmed blocking finding, do not emit that clean verdict — the lost coverage could hide exactly such a bug. Instead report the review as interrupted, exactly as the plain path does (SKILL.md Step 4's failure branch: "Review was interrupted. Please re-run the review and wait for it to complete."). Only when coverage is complete may `patch is correct` be asserted. When some findings did surface, report them and the confirmed-finding verdict normally, with the coverage caveat in the Explanation — but the "correct" verdict itself is available only when nothing failed. This mirrors the plain reviewer, which refuses to fabricate a verdict when its dispatch fails.
