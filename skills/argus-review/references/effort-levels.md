...
Subagent counts = the finders (medium 3, high/xhigh/max 5) plus ONE verifier per candidate location group plus (xhigh+) the sweep and its verifiers plus (max) 2 refuters per surviving severe finding. The verifier count scales with the number of distinct candidate locations; on a broad diff the groups are verified in severity-ordered waves of at most 3× the level's findings cap at a time (see Grouping), so concurrency stays bounded while coverage stays complete — a typical small diff runs medium ≈ 4–7 and high ≈ 6–12 total, and a broad diff runs more verifiers, just fewer at once. When announcing the review (the announcement described at the end of SKILL.md Step 3A), name the level and the honest expected scale for the diff at hand, e.g. "running a high-effort argus review (5 finder lenses + a verifier per distinct finding location) of the changes against 'main'".
